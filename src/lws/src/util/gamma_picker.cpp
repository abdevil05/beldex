...
namespace lws
{
  ...
  {
    static_assert(CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17 > 0);
    return CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17 - 1 < rct_offsets.size();
  }
  ...
  std::uint64_t gamma_picker::spendable_upper_bound() const noexcept
  {
    if (!is_valid())
      return 0;
    return *(rct_offsets.end() - CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17);
      /* Assume block indexes: [0, 1, ..., n-2, n-1]
         where n is the number of blocks in the chain
         A user can spend an output starting in block index n - CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17
         The total number of spendable outputs is the cumulative count stored at that block
         rct_offsets.end() points to index n
         Therefore we need to return index rct_offets.end() - CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17 */
  }
  ...
  std::uint64_t gamma_picker::operator()()
  {
    ...
    static_assert(CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17 > 0);
    const auto end = offsets().end() - CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE_V17 + 1;
