...
use crate::orchestrator::{
    tick, Duty, DutyExecutor, EventSource, ExecOutcome, Orchestrator, TickReport,
};
...
#[cfg(all(
    feature = "beldex-watcher",
    feature = "evm-watcher",
    feature = "tss-integration"
))]
...
mod tests {
    ...
    fn duty_backend_dispatches_by_kind() {
        ...
        assert_eq!(
            b.execute(&Duty::Release(release_ev(2))),
            ExecOutcome::Submitted
        );
    ...
    fn serve_runs_bounded_and_processes_duties() {
        let mut src = MockSource {
            mints: vec![mint_ev(1)],
            releases: vec![release_ev(2)],
        };
        let mut back = RecordingBackend {
            mints: 0,
            releases: 0,
        };
        ...
        assert_eq!(
            (back.mints, back.releases),
            (1, 1),
            "each duty handled exactly once"
        );
    }
    ...
    fn logging_backend_reports_each_duty_once_via_serve() {
        let mut src = MockSource {
            mints: vec![mint_ev(1), mint_ev(2)],
            releases: vec![release_ev(3)],
        };
        ...
        assert_eq!(
            seen.len(),
            3,
            "each of the 3 duties reported exactly once across ticks"
        );
